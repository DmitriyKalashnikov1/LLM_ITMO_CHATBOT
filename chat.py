...
if not lectureTitle == None:
    ...
    if not lectureTopic == None:
        ...
        mode = st.selectbox(label="Режим работы", index=None, options=["Обучение", "Проверка знаний"])

        if not mode == None:

            if mode == "Обучение":
                model = "qwen/qwen3-vl-4b"
                usedPrompt = prompts.studyingPrompts[lectureTopic]
            else:
                model = "qwen/qwen3-vl-8b"
                usedPrompt = prompts.testingPrompts[lectureTopic]
